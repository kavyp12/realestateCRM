...
const add = async (req, res) => {
  try {
    ...
    // Sanitize associatedListing
    const data = { ...req.body };
    if (data.associatedListing === '' || data.associatedListing === null || !mongoose.Types.ObjectId.isValid(data.associatedListing)) {
      data.associatedListing = null; // Set to null if empty or invalid
    }

    data.updatedDate = new Date();
    data.deleted = false;

    const newDocument = new ExistingModel(data);
